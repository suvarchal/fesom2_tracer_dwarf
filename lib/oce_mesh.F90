module oce_mesh_module
    ...
    ! Using MPI_REAL as fallback - data will be promoted to FP32 during communication
    ...
    ! MPI datatype for MP (mesh precision): at least single
#ifdef USE_HALF_PRECISION
...
!#include "associate_mesh_def.h"
  ...
  if (partit%npes == 1) then
     ...
     read(fileID,*) n
     error_status=0
     if (n/=partit%npes) error_status=1 !set the error status for consistency in rpart
     partit%part(1)=1
     read(fileID,*) partit%part(2:partit%npes+1)
...
!#include "associate_part_ass.h"
  ...
  ! read 2d node data
  !==============================
  ! read the nod2D from nod2d.out and check whether it is equal to partit%part(npes+1)-1
  mesh%nod2D=partit%part(partit%npes+1)-1
  allocate(mesh%coord_nod2D(2,partit%myDim_nod2D+partit%eDim_nod2D))
  if (partit%mype==0) then
    file_name=trim(meshpath)//'nod2d.out'
  ...
  if (error_status/=0) then
     write(*,*) n
     write(*,*) 'error: mesh%nod2D/=partit%part(partit%npes+1)-1'
     call par_ex(partit%MPI_COMM_FESOM, partit%mype, 1)
     STOP
  end if
  ...
  do nchunk=0, (mesh%nod2D-1)/chunk_size
     ...
     call MPI_BCast(rbuff(1:k,2), k, MPI_MP, 0, partit%MPI_COMM_FESOM, ierror)
     call MPI_BCast(ibuff(1:k,2), k, MPI_INTEGER, 0, partit%MPI_COMM_FESOM, ierror)
  ...
  if (partit%mype==0) close(fileID)
    ...
    if ((partit%mype==0) .and. (force_rotation) .and. (flag_checkisrot==1) .and. (.not. toy_ocean)) then
        ...
        write(*,*) '        oce_mesh.F90:line231 and comment this partit%part'
        write(*,*)
    ...
    elseif ((partit%mype==0) .and. (.not. force_rotation) .and. (flag_checkmustrot==1) .and. (.not. toy_ocean)) then
        write(*,*)
        print *, achar(27)//'[33m'
        write(*,*) '____________________________________________________________________'
        ...
        write(*,*) '        oce_mesh.F90:line248 and comment this partit%part'
        write(*,*)
        write(*,*) '        --> check your namelist.config !!!'
        write(*,*) '____________________________________________________________________'
        print *, achar(27)//'[0m'
  ...
  do nchunk=0, (mesh%elem2D-1)/chunk_size
     ...
     call MPI_BCast(ibuff(1:k,1), k, MPI_INTEGER, 0, partit%MPI_COMM_FESOM, ierror)
     call MPI_BCast(ibuff(1:k,2), k, MPI_INTEGER, 0, partit%MPI_COMM_FESOM, ierror)
     call MPI_BCast(ibuff(1:k,3), k, MPI_INTEGER, 0, partit%MPI_COMM_FESOM, ierror)
 ...
 if (trim(use_depthfile)=='aux3d') then
    ...
    !___________________________________________________________________________
        ...
        if (partit%mype==0) then
            write(*,*) '____________________________________________________________________'
            write(*,*) ' ERROR: You want to use aux3d.out file to define your mesh%depth, but '
            write(*,*) '        the file seems not to exist'
            write(*,*) '        --> check in namelist.config, the flag use_depthfile must be'
            write(*,*) '            use_depthfile= "aux3d" or "mesh%depth@" '
 ...
 elseif (trim(use_depthfile)=='mesh%depth@') then
    ...
    !___________________________________________________________________________
        ...
        if (partit%mype==0) then
            write(*,*) '____________________________________________________________________'
            write(*,*) ' ERROR: You want to use mesh%depth@elem.out or mesh%depth@node.out file, therefore'
            write(*,*) '        you also need the file depth_zlev.out which contains the model '
            write(*,*) '        number of layers and the mesh%depth of model levels. This file seems '
            write(*,*) '        not to exist'
            write(*,*) '        --> check in namelist.config, the flag use_depthfile must be'
            write(*,*) '            use_depthfile= "aux3d" or "mesh%depth@" and your meshfolder'
            write(*,*) '        --> model stops here'
            write(*,*) '____________________________________________________________________'
            call par_ex(partit%MPI_COMM_FESOM, partit%mype, 1)
        end if
    ...
    if (file_exist) then
        ...
        if (partit%mype==0) then
            ...
            write(*,*) '        define your mesh%depth, but the file seems not to exist'
            write(*,*) '        --> check in namelist.config, the flag use_depthfile must be'
            write(*,*) '            use_depthfile= "aux3d" or "mesh%depth@" and your meshfolder '
            write(*,*) '        --> model stops here'
            write(*,*) '____________________________________________________________________'
 ...
 if (use_depthonelem) then
    ...
    do nchunk=0, (mesh%elem2D-1)/chunk_size
        ...
        do n=1, k
            ...
            if (x>mesh%zbar(thers_zbar_lev)) x=mesh%zbar(thers_zbar_lev) !threshold for mesh%depth
            if (mapping(n)>0) then
                mesh_check=mesh_check+1
...
if ((partit%mype==0) .and. (flag_wrongaux3d==1)) then
    ...
    write(*,*) '        aux3d.out contains the number of mesh%depth layers, the mesh%depth'
    write(*,*) '        layers and the bottom mesh%depth of your mesh vertices.'
 ...
 if (partit%com_nod2D%rPEnum > MAX_NEIGHBOR_PARTITIONS) then
...
!!$ ALLOCATE(com_nod2D%rPE(com_nod2D%rPEnum))
 read(fileID,*) partit%com_nod2D%rPE(1:partit%com_nod2D%rPEnum)
!!$  ALLOCATE(com_nod2D%rptr(com_nod2D%rPEnum+1))
 read(fileID,*) partit%com_nod2D%rptr(1:partit%com_nod2D%rPEnum+1)
...
!!$  ALLOCATE(com_nod2D%sPE(com_nod2D%sPEnum))
 read(fileID,*) partit%com_nod2D%sPE(1:partit%com_nod2D%sPEnum)
!!$  ALLOCATE(com_nod2D%sptr(com_nod2D%sPEnum+1))
 read(fileID,*) partit%com_nod2D%sptr(1:partit%com_nod2D%sPEnum+1)
...
!!$  ALLOCATE(com_elem2D%rPE(com_elem2D%rPEnum))
 read(fileID,*) partit%com_elem2D%rPE(1:partit%com_elem2D%rPEnum)
...
!!$  ALLOCATE(com_elem2D%sPE(com_elem2D%sPEnum))
 read(fileID,*) partit%com_elem2D%sPE(1:partit%com_elem2D%sPEnum)
!!$  ALLOCATE(com_elem2D%sptr(com_elem2D%sPEnum+1))
 read(fileID,*) partit%com_elem2D%sptr(1:partit%com_elem2D%sPEnum+1)
...
!!$  ALLOCATE(com_elem2D_full%rPE(com_elem2D_full%rPEnum))
 read(fileID,*) partit%com_elem2D_full%rPE(1:partit%com_elem2D_full%rPEnum)
...
!!$  ALLOCATE(com_elem2D_full%sPE(com_elem2D_full%sPEnum))
 read(fileID,*) partit%com_elem2D_full%sPE(1:partit%com_elem2D_full%sPEnum)
!!$  ALLOCATE(com_elem2D_full%sptr(com_elem2D_full%sPEnum+1))
 read(fileID,*) partit%com_elem2D_full%sptr(1:partit%com_elem2D_full%sPEnum+1)
 n=partit%com_elem2D_full%sptr(partit%com_elem2D_full%sPEnum+1)-1
...
! such that sum(myDim_elem2D_shrinked) over all processors will give elem2D
 ...
 DO n=1, partit%myDim_elem2D
    if (mesh%elem2D_nodes(1, n) > partit%myDim_nod2D) cycle
...
!#include "associate_part_ass.h"
    ...
    if (mesh_check/=partit%myDim_elem2D+partit%eDim_elem2D+partit%eXDim_elem2D) then
        ...
        print *, achar(27)//'[33m'
        write(*,*) '____________________________________________________________________'
        write(*,*) ' ERROR: while reading elvls.out on partit%mype=', partit%mype
        write(*,*) '        ',mesh_check, ' values have been read in according to partitioning'
        write(*,*) '        it does not equal to partit%myDim_elem2D+partit%eDim_elem2D = ', partit%myDim_elem2D+partit%eDim_elem2D
        write(*,*) '____________________________________________________________________'
        print *, achar(27)//'[0m'
    end if
    ...
    if (mesh_check/=partit%myDim_nod2D+partit%eDim_nod2D) then
        ...
        print *, achar(27)//'[33m'
        write(*,*) '____________________________________________________________________'
        write(*,*) ' ERROR: while reading nlvls.out on partit%mype=', partit%mype
        write(*,*) '        ',mesh_check, ' values have been read in according to partitioning'
        write(*,*) '        it does not equal to partit%myDim_nod2D+partit%eDim_nod2D = ', partit%myDim_nod2D+partit%eDim_nod2D
        write(*,*) '____________________________________________________________________'
        print *, achar(27)//'[0m'
    ...
    if (partit%mype==0) then
        ...
        write(*,*) ' --> Min/max mesh%depth on partit%mype : ', partit%mype, -mesh%zbar(minval(mesh%nlevels)),-mesh%zbar(maxval(mesh%nlevels))
    endif
...
!!PS     allocate(mesh%cavity_flag_n(myDim_nod2D+eDim_nod2D))
    ...
    if (mesh_check/=partit%myDim_elem2D+partit%eDim_elem2D+partit%eXDim_elem2D) then
        ...
        print *, achar(27)//'[33m'
        write(*,*) '____________________________________________________________________'
        write(*,*) ' ERROR: while reading cavity_elvls.out on partit%mype=', partit%mype
        write(*,*) '        ',mesh_check, ' values have been read in according to partitioning'
        write(*,*) '        it does not equal to partit%myDim_elem2D+partit%eDim_elem2D = ', partit%myDim_elem2D+partit%eDim_elem2D
        write(*,*) '____________________________________________________________________'
        print *, achar(27)//'[0m'
    ...
    if (mesh_check/=partit%myDim_nod2D+partit%eDim_nod2D) then
        ...
        print *, achar(27)//'[33m'
        write(*,*) '____________________________________________________________________'
        write(*,*) ' ERROR: while reading cavity_nlvls.out on partit%mype=', partit%mype
        write(*,*) '        ',mesh_check, ' values have been read in according to partitioning'
        write(*,*) '        it does not equal to partit%myDim_nod2D+partit%eDim_nod2D = ', partit%myDim_nod2D+partit%eDim_nod2D
        write(*,*) '____________________________________________________________________'
        print *, achar(27)//'[0m'
...
!!PS     end if
    ...
    if (partit%mype==0)  then
        ...
        if (file_exist) then
            ...
            write(*,*) '        Wrong mesh path ? This file provides the necessary mesh%depth'
    ...
    if (use_cavityonelem) then
        ...
        if (mesh_check/=partit%myDim_elem2D+partit%eDim_elem2D+partit%eXDim_elem2D) then
            ...
            print *, achar(27)//'[33m'
            write(*,*) '____________________________________________________________________'
            write(*,*) ' ERROR: while reading mesh%cavity_depth.out on partit%mype=', partit%mype
            write(*,*) '        ',mesh_check, ' values have been read in according to partitioning'
            write(*,*) '        it does not equal to partit%myDim_elem2D+partit%eDim_elem2D+partit%eXDim_elem2D = ', partit%myDim_elem2D+partit%eDim_elem2D+partit%eXDim_elem2D
            write(*,*) '____________________________________________________________________'
            print *, achar(27)//'[0m'
...
!#include "associate_part_ass.h"
    ...
    allocate(mesh%ulevels_nod2D_max(partit%myDim_nod2D+partit%eDim_nod2D))
...
! 0 proc reads the data and sends it to other procs
 ...
 if (partit%mype==0) then
  ...
  call MPI_BCast(mesh%edge2D,    1, MPI_INTEGER, 0, partit%MPI_COMM_FESOM, ierror)
  call MPI_BCast(mesh%edge2D_in, 1, MPI_INTEGER, 0, partit%MPI_COMM_FESOM, ierror)

  allocate(mesh%edges(2,partit%myDim_edge2D+partit%eDim_edge2D))
  allocate(mesh%edge_tri(2,partit%myDim_edge2D+partit%eDim_edge2D))
 ...
 do nchunk=0, (mesh%edge2D-1)/chunk_size
    !create the mapping for the current chunk
    mapping(1:chunk_size)=0
    do n=1, partit%myDim_edge2D+partit%eDim_edge2D
       ipos=(partit%myList_edge2D(n)-1)/chunk_size
       if (ipos==nchunk) then
          iofs=partit%myList_edge2D(n)-nchunk*chunk_size
          mapping(iofs)=n
       end if
    end do
    !read the chunk into the buffers
    ...
    call MPI_BCast(ibuff(1:4,1:k), 4*k, MPI_INTEGER, 0, partit%MPI_COMM_FESOM, ierror)
...
!    call MPI_BCast(ibuff(1:k,4), k, MPI_INTEGER, 0, MPI_COMM_FESOM, ierror)
 ...
 if (mesh_check/=partit%myDim_edge2D+partit%eDim_edge2D) then
    write(*,*) 'ERROR while reading mesh%edges.out/mesh%edge_tri.out on partit%mype=', partit%mype
    write(*,*) mesh_check, ' values have been read in according to partitioning'
...
DO n=1, partit%myDim_edge2D+partit%eDim_edge2D
   DO k=1,2
	  ...
	  if((q>0).and.(q<=partit%myDim_elem2D)) then
	  aux(q)=aux(q)+1
...
CALL MPI_BARRIER(partit%MPI_COMM_FESOM, partit%MPIerr)
 ...
 mymax=0
 rmax=0
 mymax(partit%mype+1)=maxval(mesh%nod_in_elem2D_num(1:partit%myDim_nod2D))
 call MPI_AllREDUCE( mymax, rmax, &
       partit%npes, MPI_INTEGER,MPI_SUM, &
 ...
 allocate (temp_i(partit%myDim_nod2D+partit%eDim_nod2D))
 temp_i=0
 DO n=1, maxval(rmax)
       ! Exchange global element numbers
       do j=1,partit%myDim_nod2D
         if (mesh%nod_in_elem2D(n,j)>0) temp_i(j)=partit%myList_elem2D(mesh%nod_in_elem2D(n,j))
       enddo
...
DO elem=1,partit%myDim_elem2D
   ...
   END DO
   if (elem1<2) then
    write(*,*) 'Insufficient number of neighbors ', partit%myList_elem2D(elem)
    call par_ex(partit%MPI_COMM_FESOM, partit%mype, 1)
    STOP
   end if
END DO
...
#if defined (__oasis)
    allocate(mesh%x_corners(partit%myDim_nod2D, maxval(rmax)))
    allocate(mesh%y_corners(partit%myDim_nod2D, maxval(rmax)))
    DO n=1, partit%myDim_nod2D
       DO j=1, mesh%nod_in_elem2D_num(n)
...
real(kind=MP),  intent(inout)  :: x, y
type(t_mesh),   intent(in), target :: mesh
real(kind=MP)                  :: a(2), b(2)

a=mesh%coord_nod2D(:,n1)
b=mesh%coord_nod2D(:,n2)
if(a(1)-b(1)> cyclic_length/2.0_MP) a(1)=a(1)-cyclic_length
if(a(1)-b(1)<-cyclic_length/2.0_MP) b(1)=b(1)-cyclic_length
x=0.5_MP*(a(1)+b(1))
y=0.5_MP*(a(2)+b(2))
...
real(kind=MP)                ::  ax(3), amin

   elnodes=mesh%elem2D_nodes(:,elem)
   ax=mesh%coord_nod2D(1,   elnodes)
   amin=minval(ax)
   DO k=1,3
   if(ax(k)-amin>=cyclic_length/2.0_MP) ax(k)=ax(k)-cyclic_length
   if(ax(k)-amin<-cyclic_length/2.0_MP) ax(k)=ax(k)+cyclic_length
   END DO
   x=sum(ax)/3.0_MP
   y=sum(mesh%coord_nod2D(2,elnodes))/3.0_MP
...
SUBROUTINE mesh_areas(partit, mesh)
    ...
    real(kind=MP)                             :: a(2), b(2), ax, ay, lon, lat
    real(kind=MP)                             :: vol, vol2
    real(kind=MP), allocatable,dimension(:)   :: work_array
...
!#include "associate_part_ass.h"
    ...
    do n=1, partit%myDim_elem2D
        elnodes=mesh%elem2D_nodes(:,n)
        ay=sum(mesh%coord_nod2D(2,elnodes))/3.0_MP
        ay=cos(ay)
        if (cartesian) ay=1.0_MP
        a = mesh%coord_nod2D(:,elnodes(2))-mesh%coord_nod2D(:,elnodes(1))
        b = mesh%coord_nod2D(:,elnodes(3))-mesh%coord_nod2D(:,elnodes(1))
        call trim_cyclic_mp(a(1))
        call trim_cyclic_mp(b(1))
        a(1)=a(1)*ay
        b(1)=b(1)*ay
        mesh%elem_area(n)=0.5_MP*abs(a(1)*b(2)-b(1)*a(2))
    end do
    ...
    do n=1,partit%myDim_nod2D+partit%eDim_nod2D
        ...
        do nz=nzmin,nzmax
!!PS             mesh%area_inv(nz,n) = 1._MP/mesh%area(nz,n)
            if (mesh%area(nz,n) > 0._MP) then
                mesh%area_inv(nz,n) = 1._MP/mesh%area(nz,n)
            else
                mesh%area_inv(nz,n) = 0._MP
            end if
        end do
    end do

    if (use_cavity) then
        mesh%areasvol_inv = 0.0_MP
        do n=1,partit%myDim_nod2D+partit%eDim_nod2D
            nzmin = mesh%ulevels_nod2D(n)
            nzmax = mesh%nlevels_nod2D(n)-1
            do nz=nzmin,nzmax
!!PS                 mesh%areasvol_inv(nz,n) = 1._MP/mesh%areasvol(nz,n)
                if (mesh%areasvol(nz,n) > 0._MP) then
                    mesh%areasvol_inv(nz,n) = 1._MP/mesh%areasvol(nz,n)
                else
                    mesh%areasvol_inv(nz,n) = 0._MP
                end if
            end do
        end do
    else
    ...
    allocate(work_array(partit%myDim_nod2D))
    !!PS mesh%mesh_resolution=sqrt(mesh%area(1, :)/pi)*2._MP
    do n=1,partit%myDim_nod2D+partit%eDim_nod2D
        mesh%mesh_resolution(n)=sqrt(mesh%areasvol(mesh%ulevels_nod2D(n),n)/pi)*2._MP
    end do
    ...
    do q=1, 3 !apply mass matrix N times to smooth the field
        do n=1, partit%myDim_nod2D
            ...
            do j=1, mesh%nod_in_elem2D_num(n)
                elem=mesh%nod_in_elem2D(j, n)
                elnodes=mesh%elem2D_nodes(:,elem)
                work_array(n)=work_array(n)+sum(mesh%mesh_resolution(elnodes))/3._MP*mesh%elem_area(elem)
...
!!PS         vol=vol+mesh%area(1, n) ! area only surface
    ...
    call MPI_AllREDUCE(vol, mesh%ocean_area, 1, MPI_MP, MPI_SUM, &
        partit%MPI_COMM_FESOM, partit%MPIerr)
    ...
    if (partit%mype==0) then
        write(*,*) '____________________________________________________________________'
        write(*,*) ' --> mesh statistics:', partit%mype
        write(*,*)  partit%mype, 'maxArea ',maxval(mesh%elem_area), '   MinArea ', minval(mesh%elem_area)
        write(*,*)  partit%mype, 'maxScArea ',maxval(mesh%area(1,:)), &
                    '   MinScArea ', minval(mesh%area(1,:))
        write(*,*)  partit%mype, 'Edges:    ', mesh%edge2D, ' internal ', mesh%edge2D_in
        if (partit%mype==0) then
            write(*,*) '     > Total ocean surface mesh%area is           : ', mesh%ocean_area, ' m^2'
            write(*,*) '     > Total ocean surface mesh%area with cavity is: ', mesh%ocean_areawithcav, ' m^2'
        end if
    endif
...
! gradient_elem(6,partit%myDim_elem2D)
...
real(kind=MP)	     :: a(2), b(2), ax, ay
real(kind=WP)        :: lon, lat
real(kind=MP)	     :: dfactor
real(kind=MP)	     :: deltaX31, deltaX21, deltaY31, deltaY21
real(kind=MP)        :: x(3), y(3), cxx, cxy, cyy, d
real(kind=MP), allocatable :: center_x(:), center_y(:), temp(:)
real(kind=MP)              :: t0, t1
integer                    :: i, nn, ns
type(t_mesh),   intent(inout), target :: mesh
type(t_partit), intent(inout), target :: partit
...
t0=MPI_Wtime()
 ...
 allocate(mesh%edge_dxdy(2,partit%myDim_edge2D+partit%eDim_edge2D))
 allocate(mesh%edge_cross_dxdy(4,partit%myDim_edge2D+partit%eDim_edge2D))
 allocate(mesh%gradient_sca(6,partit%myDim_elem2D))
 allocate(mesh%gradient_vec(6,partit%myDim_elem2D))
 allocate(mesh%metric_factor(partit%myDim_elem2D+partit%eDim_elem2D+partit%eXDim_elem2D))
 allocate(mesh%elem_cos(partit%myDim_elem2D+partit%eDim_elem2D+partit%eXDim_elem2D))
 allocate(mesh%coriolis(partit%myDim_elem2D))
 ...
 ! ============
 DO n=1,partit%myDim_nod2D+partit%eDim_nod2D
    call r2g(lon, lat, real(mesh%coord_nod2D(1,n),WP), real(mesh%coord_nod2D(2,n),WP))
...
#endif
 if (cartesian) then
    mesh%elem_cos=1.0_MP
    mesh%metric_factor=0.0_MP
 end if
 ...
 DO n=1, partit%myDim_edge2D+partit%eDim_edge2D    !!! Difference to FESOM2:
   ...
   if(el(2)>0) then
      ...
      mesh%edge_cross_dxdy(3:4,n)=0.0_MP
   end if
 END DO
...
real(kind=MP)	            :: vol_n(mesh%nl), vol_e(mesh%nl), aux(mesh%nl)
!#include "associate_part_def.h"
!#include "associate_part_ass.h"
   ...
   call MPI_AllREDUCE(aux, vol_n, mesh%nl, MPI_MP, MPI_SUM, &
       partit%MPI_COMM_FESOM, partit%MPIerr)

   aux=0._MP
   do elem=1, partit%myDim_elem2D
      elnodes=mesh%elem2D_nodes(:, elem)
      if (elnodes(1) > partit%myDim_nod2D) CYCLE
      do nz=mesh%ulevels(elem), mesh%nlevels(elem)-1
         aux(nz)=aux(nz)+mesh%elem_area(elem)
      end do
   end do
   call MPI_AllREDUCE(aux, vol_e, mesh%nl, MPI_MP, MPI_SUM, &
       partit%MPI_COMM_FESOM, partit%MPIerr)
...
!#include "associate_mesh_ass.h"
    ...
    do n=1, partit%myDim_nod2D
        do nz=mesh%ulevels_nod2D(n), mesh%nlevels_nod2D(n)-1
            aux=aux+mesh%areasvol(nz, n)*mesh%hnode(nz,n)
        end do
    end do
    call MPI_AllREDUCE(aux, vol_n, 1, MPI_MP, MPI_SUM, partit%MPI_COMM_FESOM, partit%MPIerr)
    !___________________________________________________________________________
    ...
    do elem=1, partit%myDim_elem2D
        ...
        if (elnodes(1) > partit%myDim_nod2D) cycle
        do nz=mesh%ulevels(elem), mesh%nlevels(elem)-1
            aux=aux+mesh%elem_area(elem)*mesh%helem(nz,elem)
        end do
    end do
    call MPI_AllREDUCE(aux, vol_e, 1, MPI_MP, MPI_SUM, partit%MPI_COMM_FESOM, partit%MPIerr)
