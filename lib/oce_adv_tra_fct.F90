...
!#include "associate_mesh_ass.h"
    ...
    my_size=partit%myDim_nod2D+partit%eDim_nod2D
    allocate(twork%fct_LO(mesh%nl-1, my_size))        ! Low-order solution
    allocate(twork%adv_flux_hor(mesh%nl-1,partit%myDim_edge2D)) ! antidiffusive hor. contributions / from mesh%edges
    allocate(twork%adv_flux_ver(mesh%nl, partit%myDim_nod2D))   ! antidiffusive ver. fluxes / from nodes

    allocate(twork%fct_ttf_max(mesh%nl-1, my_size),twork%fct_ttf_min(mesh%nl-1, my_size))
    allocate(twork%fct_plus(mesh%nl-1, my_size),   twork%fct_minus(mesh%nl-1, my_size))
    ! Initialize with zeros:
    twork%fct_LO=0.0_MP
    twork%adv_flux_hor=0.0_MP
    twork%adv_flux_ver=0.0_MP
    twork%fct_ttf_max=0.0_MP
    twork%fct_ttf_min=0.0_MP
    twork%fct_plus=0.0_MP
    twork%fct_minus=0.0_MP

    if (partit%mype==0) write(*,*) 'FCT is initialized'
...
subroutine oce_tra_adv_fct(dt, ttf, lo, adf_h, adf_v, fct_ttf_min, fct_ttf_max, fct_plus, fct_minus, AUX, partit, mesh)
    ...
    type(t_mesh),  intent(in),    target :: mesh
    type(t_partit),intent(inout), target :: partit
    real(kind=MP), intent(inout)      :: fct_ttf_min(mesh%nl-1, partit%myDim_nod2D+partit%eDim_nod2D)
    real(kind=MP), intent(inout)      :: fct_ttf_max(mesh%nl-1, partit%myDim_nod2D+partit%eDim_nod2D)
    real(kind=WP), intent(in)         :: ttf(mesh%nl-1, partit%myDim_nod2D+partit%eDim_nod2D)
    real(kind=MP), intent(in)         :: lo (mesh%nl-1, partit%myDim_nod2D+partit%eDim_nod2D)
    real(kind=MP), intent(inout)      :: adf_h(mesh%nl-1, partit%myDim_edge2D)
    real(kind=MP), intent(inout)      :: adf_v(mesh%nl,   partit%myDim_nod2D)
    ...
    integer                           :: n, nz, k, elem, enodes(3), num, el(2), nl1, nl2, nu1, nu2, nl12, nu12, edge, ic, ie
...
!$OMP DO
    ...
    do n=1,partit%myDim_nod2D
        ...
        !$ACC LOOP VECTOR
        do nz=nu1,nl1-1
            flux=fct_plus(nz,n)*dt/mesh%areasvol(nz,n)+flux_eps
            fct_plus(nz,n)=min(1.0_WP,fct_ttf_max(nz,n)/flux)
            flux=fct_minus(nz,n)*dt/mesh%areasvol(nz,n)-flux_eps
